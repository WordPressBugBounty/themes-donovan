	...
	if ( 'primary' === $args->theme_location || 'secondary' === $args->theme_location ) {
		foreach ( $item->classes as $value ) {
			if ( 'menu-item-has-children' === $value || 'page_item_has_children' === $value ) {
				$title = $title . donovan_get_svg( 'expand' );
			}
		}
	}
...
function donovan_social_icons_menu( $item_output, $item, $depth, $args ) {
	...
	// Get SVG.
	$svg = apply_filters( 'donovan_get_social_svg', donovan_get_social_svg( $icon ), $item_output );
...
function donovan_supported_social_icons() {
	...
	return apply_filters( 'donovan_supported_social_icons', $supported_social_icons );
}
